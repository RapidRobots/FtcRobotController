...
@TeleOp(name="TeleOp DrivePowerPlayGame")
public class DrivePowerPlayGame extends OpMode {
    ...
    /**
     * 
    private void moveArmUpAsync(int holdPosition) {
        ...
        try {
            ...
            moveArmTask.setPosition(holdPosition);
            taskT.start();
        }catch(Exception ex) {
    ...
    */
    ...
    private void sendTelemetry(boolean straight) {
        ...
        if (straight) {
            ...
            telemetry.addData("Digital Touch Left>", !leftLimitSwitch.getState());
            telemetry.addData("Digital Touch Right>", !rightLimitSwitch.getState());
        } else {
   ...
   /**
    ...
    private void limitArmDownMove(){
        ...
        if (((leftLimitSwitch != null &&
                !leftLimitSwitch.getState()) || (rightLimitSwitch != null &&
                !rightLimitSwitch.getState())) &&
                arm_lift_speed > 0){
            arm_lift_speed = 0;
            armLeftMotor.setMode(DcMotor.RunMode.RESET_ENCODERS);
            armLeftMotor.setMode(DcMotor.RunMode.RUN_USING_ENCODER);
